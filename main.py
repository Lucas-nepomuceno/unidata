...
@st.cache_data
def load_data():
    return pd.read_csv('bd_para_analise.csv', index_col=0)
...
if st.session_state.page == "home":
    ...
    <ul>
        ...
        <span style="color: lightgreen;">{df.shape[0]}</span></li>
        <li><strong>Quantidade de Pessoas que ativaram o sinistro:</strong> 
        <span style="color: lightgreen;">{df['segurado'].nunique()}</span></li>
        <li><strong>Quantidade Média de Sinistro por Pessoa:</strong> 
        <span style="color: lightgreen;">{round(df.shape[0]/df['segurado'].nunique(), 2)}</span></li>
        <li><strong>Máximo Valor Pago:</strong> 
        <span style="color: lightgreen;">{round(df['valor_pago_sinistro'].max(), 0)}</span></li>
        <li><strong>Prestador Mais Frequente:</strong> 
        <span style="color: lightgreen;">{df['nome_prestador_sinistro'].mode()[0]}</span></li>
    </ul>
    """, unsafe_allow_html=True)
    ...
    chart_quantidade_cluster = alt.Chart(data_quantidade_cluster).mark_bar().encode(
        ...
        color=alt.Color('cluster:N', title='Perfil', scale=alt.Scale(
                            domain=['1', '2', '3'],
                            range=['#0a3200', '#008a25', 'lightgreen']
                        )),
    ).properties(
    ...
    chart_categoria_cluster = alt.Chart(top_3_categorias_por_cluster).mark_bar().encode(
        ...
        title='Distribuição de Categorias por Cluster'
    )
    ...
    chart_prestador_cluster = alt.Chart(top_3_prestadores_por_cluster).mark_bar().encode(
        ...
        title='Distribuição de Prestadores por Cluster'
    )
    ...
    chart_valor_cluster = alt.Chart(df_clusters_valor_pago).mark_bar().encode(
        ...
        color=alt.Color('cluster:N', title='Cluster', scale=alt.Scale(
            domain=['1', '2', '3'],
            range=['#0a3200', '#008a25', 'lightgreen']
        )),
        tooltip=['cluster:O', 'valor_pago:Q']  # Garantir que a quantidade esteja no tooltip como úmero
    ).properties(
        title='Distribuição da Média do Valor Pago por Cluster'
    )
